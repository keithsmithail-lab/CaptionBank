...
(function () {
  ...
  const HOOKS = [
    "Real talk: ",
    "Quick win: ",
    "If you struggle with this, read on: ",
    "Hot take: ",
    "Save this for later: ",
  ...
  const BODIES = {
    generic: [
      ...
      "this works even if you feel stuck or short on time.",
  ...
  function buildHashtags() {
    ...
    const base = [];
    for (const w of words) { if (!stops.has(w) && base.length < 3) base.push("#" + w.replace(/[^a-z0-9]/g, "")); }
    const platformTags = {
  ...
  // SAFER splitter: no lookbehind, works in all modern browsers
  ...
  function makeFromNiche() {
    ...
    const base = applyTone(hook + body);
    const long = expandLength(base);
    const emoji = state.useEmojis ? " " + pick(GENERIC_EMOJIS) : "";
    const cta = state.useCTA ? " " + pick(CTAS) : "";
    const tags = state.useHashtags ? "\n\n" + buildHashtags() : "";
    const caption = (long + emoji + cta + tags).trim();
    const canva = makeCanvaPrompt(hook);
    return caption + "\n\nCanva image prompt:\n" + canva;
  }
  ...
  function makeFromArticle(points, i) {
    ...
    const tags = state.useHashtags ? "\n\n" + buildHashtags() : "";
    const caption = (long + emoji + cta + tags).trim();
    const canva = makeCanvaPrompt(hook, topic);
    return caption + "\n\nCanva image prompt:\n" + canva;
  }
  ...
  function render() {
    ...
    state.captions.forEach((c, i) => {
      ...
      bCopy.className = "btn alt";
      bCopy.textContent = "Copy";
      bCopy.onclick = () => { navigator.clipboard.writeText(c); bCopy.textContent = "Copied"; setTimeout(() => bCopy.textContent = "Copy", 900); };
  ...
  $("#generate").addEventListener("click", () => {
    ...
    if (state.mode === "rewrite") {
      ...
      state.captions = Array.from({ length: n }, (_, i) => makeFromArticle(pts, i));
    } else {
      state.captions = Array.from({ length: n }, makeFromNiche);
    }
    render();
  });
  ...
  $("#downloadCSV").addEventListener("click", () => {
    ...
    const rows = state.captions.map((c, i) => [
      i + 1, state.mode, state.niche || "", state.platform, state.tone, state.length, state.useCTA ? "On" : "Off", c,
    ]);
    const csv = [header, ...rows].map((r) => r.map((x) => `"${String(x).replaceAll('"', '""')}"`).join(",")).join("\n");
    const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = "captions_" + Date.now() + ".csv"; a.click();
  ...
  function openResearch(which) {
    ...
    const g = `https://www.google.com/search?q=${q(core)}&tbm=vid`;
    const r = `https://www.google.com/search?q=${q(core + " site:reddit.com")}`;
    const t = `https://www.google.com/search?q=${q(core + " site:tiktok.com")}`;
    const i = `https://www.google.com/search?q=${q(core + " site:instagram.com")}`;
    if (which === "google") window.open(g, "_blank");
    if (which === "reddit") window.open(r, "_blank");
    if (which === "tiktok") window.open(t, "_blank");
    if (which === "instagram") window.open(i, "_blank");
  }
  ...
  async function fetchArticleIntoTextarea() {
    ...
    try {
      ...
      status.textContent = "Fetch failed. Check the URL or CORS policy.";
    }
  }
