...
(function(){
  ...
  const state = { mode:"niche", niche:"", articleText:"", platform:"Instagram", tone:"Friendly", length:"Medium", useEmojis:true, useHashtags:true, useCTA:true, styleText:"", qty:5, captions:[] };
  ...
  const BODIES={generic:["here is a simple way to take action in minutes.","try this 3 step approach and thank me later.","this works even if you feel stuck or short on time.","use this today and tell me what changes for you.","this is what I wish I knew when I started."],
  ...
  const lengthen=t=> state.length==="Short"?t : state.length==="Medium"? t+" Here is how to get started today." : t+" Step 1: get clear on the outcome. Step 2: use a simple checklist. Step 3: review and improve tomorrow.";
  function tags(){
    ...
    const plat={Instagram:["#instagramtips","#contentcreator"],Facebook:["#facebookmarketing","#socialmediamarketing"],TikTok:["#tiktoktips","#smallbusinesstiktok"],LinkedIn:["#linkedinmarketing","#b2b"]}[state.platform];
    return [...base,...plat].join(" ");
  }
  ...
  function splitPoints(text){
    ...
    const limited=sents.slice(0,40);
    const scored=limited.map(s=>{const len=Math.min(s.length,200); const num=(s.match(/\d+/g)||[]).length; const verbs=(s.match(/\b(is|are|has|have|can|should|will|avoid|use|get|make|save|protect|plan)\b/gi)||[]).length; return {s:s.trim(),score:len*0.3+num*2+verbs};});
    scored.sort((a,b)=>b.score-a.score);
    return scored.slice(0,10).map(x=>x.s);
  }

  function canva(hook,topic){
  ...
  function fromNiche(){
    ...
    const base=tone(hook+body), long=lengthen(base), e=state.useEmojis?" "+pick(EMOJI):"", cta=state.useCTA?" "+pick(CTAS):"", t=state.useHashtags?"\n\n"+tags():"";
    const caption=(long+e+cta+t).trim();
    return caption+"\n\nCanva image prompt:\n"+canva(hook);
  }
  ...
  function render(){
    // adapt UI per mode
    const label=$("#articleLabel"), ta=$("#articleText"), row=$("#fetchRow"), hint=$("#modeHint");
    if(state.mode==="rewrite"){ label.textContent="Article text (paste or fetch below)"; ta.placeholder="Paste the article text or click Fetch from URL..."; row.style.display=""; hint.textContent="Tip: Paste article text or use Fetch from URL (some sites block bots)."; }
    else if(state.mode==="ig"){ label.textContent="Instagram caption text (paste caption only)"; ta.placeholder="Open the Instagram post, copy the caption text, and paste it here."; row.style.display="none"; hint.textContent="Tip: Instagram blocks automated fetching. Paste the caption text manually."; }
    else { hint.textContent="Tip: In rewrite modes you can paste text, or (for articles) fetch a URL via the built-in serverless function."; }

    const out=$("#output"); out.innerHTML="";
    if(!state.captions.length){
      const ph=document.createElement("div"); ph.className="placeholder"; ph.textContent="Your captions will appear here after you click Generate."; out.appendChild(ph);
      $("#crumbs").textContent=""; return;
    }
    state.captions.forEach((c,i)=>{
      const card=document.createElement("div"); card.className="cardOut";
      const head=document.createElement("div"); head.className="cardHead"; head.textContent="Caption "+(i+1);
      const txt=document.createElement("textarea"); txt.className="text"; txt.rows=12; txt.readOnly=true; txt.value=c;
      const row=document.createElement("div"); row.className="rowBtns";
      const b=document.createElement("button"); b.className="btn alt"; b.textContent="Copy";
      b.onclick=()=>{navigator.clipboard.writeText(c); b.textContent="Copied"; setTimeout(()=>b.textContent="Copy",900);};
      row.appendChild(b); card.appendChild(head); card.appendChild(txt); card.appendChild(row); out.appendChild(card);
    });
    const modeText = state.mode==="niche" ? (state.niche||"Any niche") : (state.mode==="ig" ? "Rewriting IG caption" : "Rewriting from article");
  ...
  $("#generate").addEventListener("click",()=>{
    ...
    if(state.mode==="rewrite"||state.mode==="ig"){
      const raw=splitPoints(state.articleText);
  ...
  function openResearch(which){
    ...
    const map={google:`https://www.google.com/search?q=${q(core)}&tbm=vid`,
  ...
  // serverless fetch (articles only)
  async function fetchArticle(){
    ...
    }catch(e){ status.textContent="Fetch failed. Site may block bots (CORS/anti-scrape). Paste the text instead."; }
  }
